...
const MAX_RAW = 50; // 10 graded items × max 5 points each
const MAX_SCORE = 20;

const CompanySupervisorDashboard = ({ user, onLogout }) => {
  ...
  // Scale raw score (0–50) to out of 20
  ...
  const handleSubmit = async () => {
    ...
    try {
      setNotice(null);
      setSubmitting(true);
      const response = await fetch(`${API_BASE}/api/grade/company`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          indexNumber,
          grades,
          totalScore: scaledScore, // submitted as a value out of 20
          comments: '',
        }),
      });
  ...
  return (
    <div>
      <div className="header">
        ...
        <div className="welcome-user">Welcome, Company Supervisor</div>
      </div>
              ...
              <tbody>
                    ...
                    {[0, 1, 2, 3, 4, 5].map(grade => (
                      <td key={grade}>
                        <input
                          ...
                          name={`skill-${num}`}
                          value={grade}
                          onChange={(e) => handleGradeChange(`skill-${num}`, e.target.value)}
                        />
                      </td>
                    ))}
                  </tr>
                  ...
                  {[0, 1, 2, 3, 4, 5].map(grade => (
                    <td key={grade}>
                      <input
                        ...
                        name="complete-work"
                        value={grade}
                        onChange={(e) => handleGradeChange('complete-work', e.target.value)}
                      />
                    </td>
                  ))}
                </tr>
                  ...
                  {[0, 1, 2, 3, 4, 5].map(grade => (
                    <td key={grade}>
                      <input
                        ...
                        value={grade}
                        onChange={(e) => handleGradeChange('follow-instructions', e.target.value)}
                      />
                    </td>
                  ))}
                </tr>
                <tr>
                  ...
                  {[0, 1, 2, 3, 4, 5].map(grade => (
                    <td key={grade}>
                      <input
                        type="radio"
                        name="make-initiatives"
                        value={grade}
                        onChange={(e) => handleGradeChange('make-initiatives', e.target.value)}
                      />
                    </td>
                  ))}
                </tr>
                  ...
                  {[0, 1, 2, 3, 4, 5].map(grade => (
                    <td key={grade}>
                      <input
                        ...
                        name="teamwork"
                        value={grade}
                        onChange={(e) => handleGradeChange('teamwork', e.target.value)}
                      />
                    </td>
                  ))}
                </tr>
                  ...
                  {[0, 1, 2, 3, 4, 5].map(grade => (
                    <td key={grade}>
                      <input
                        ...
                        name="communication"
                        value={grade}
                        onChange={(e) => handleGradeChange('communication', e.target.value)}
                      />
                    </td>
                  ))}
                </tr>
                ...
                <tr style={{ borderTop: '2px solid #333', background: '#f9f9f9', fontWeight: 'bold' }}>
                  <td colSpan="7" style={{ padding: '12px 16px' }}>
                    <span>Total Score (out of 20): </span>
      ...
      <AIFAQChatbot role="company_supervisor" />
    </div>
  );
};
